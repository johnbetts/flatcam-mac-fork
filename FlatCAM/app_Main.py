...
class App(QtCore.QObject):
	...
	def on_startup_args(self, args: list[str], silent=False):
		"""
	...
	def on_about(self):
		...
		class AboutDialog(QtWidgets.QDialog):
			def __init__(self, app, parent=None):
				...
				def add_programmer(programmer: str, status: str | None = None, email: str | None = None):
					next_row = self.prog_grid_lay.rowCount()
					self.prog_grid_lay.addWidget(QtWidgets.QLabel(programmer), next_row, 0)
					if status:
						self.prog_grid_lay.addWidget(QtWidgets.QLabel(status), next_row, 1)
					if email:
						self.prog_grid_lay.addWidget(QtWidgets.QLabel(email), next_row, 2)

				def add_programmer_section():
					next_row = self.prog_grid_lay.rowCount()
					self.prog_grid_lay.addWidget(QtWidgets.QLabel(''), next_row, 0)
				...
				add_programmer('Juan Pablo Caram', _("Program Author"))
				add_programmer_section()
				add_programmer('vika-sonne', 'β Maintainer 2023')
				add_programmer("Denis Hayrullin")
				add_programmer("Kamil Sopko")
				add_programmer("Marius Stanciu", _("BETA Maintainer >= 2019"), "<marius_adrian@yahoo.com>")
				add_programmer_section()
				add_programmer("David Robertson")
				add_programmer("Matthieu Berthomé")
				add_programmer("Mike Evans")
				add_programmer("Victor Benso")
				add_programmer_section()
				add_programmer("Jørn Sandvik Nilsson")
				add_programmer("Lei Zheng")
				add_programmer("Leandro Heck")
				add_programmer("Marco A Quezada")
				add_programmer_section()
				add_programmer("Cedric Dussud")
				add_programmer("Chris Hemingway")
				add_programmer("Damian Wrobel")
				add_programmer("Daniel Sallin")
				add_programmer_section()
				add_programmer("Bruno Vunderl")
				add_programmer("Gonzalo Lopez")
				add_programmer("Jakob Staudt")
				add_programmer("Mike Smith")
				add_programmer_section()
				add_programmer("Barnaby Walters")
				add_programmer("Steve Martina")
				add_programmer("Thomas Duffin")
				add_programmer("Andrey Kultyapov")
				add_programmer_section()
				add_programmer("Alex Lazar")
				add_programmer("Chris Breneman")
				add_programmer("Eric Varsanyi")
				add_programmer("Lubos Medovarsky")
				add_programmer_section()
				add_programmer("@Idechix")
				add_programmer("@SM")
				add_programmer("@grbf")
				add_programmer("@Symonty")
				add_programmer("@mgix")
				...
				def add_components():
					...
					from svglib.svglib import __version__ as svglib_version
					from vispy import __version__ as vispy_version
					from numpy import __version__ as numpy_version
					from rtree import __version__ as rtree_version
					from rasterio import __version__ as rasterio_version
					from ezdxf import __version__ as ezdxf_version
					from simplejson import __version__ as simplejson_version
					from lxml import __version__ as lxml_version
					self.components_tab_layout.addWidget(QtWidgets.QLabel(
						'<table>'
						f'<tr><td>Python</td><td>{version_info.major}.{version_info.minor}.{version_info.micro}</td></tr>'
						f'<tr><td>Qt</td><td>{QT_VERSION_STR}</td></tr>'
						f'<tr><td>PyQt</td><td>{PYQT_VERSION_STR}</td></tr>'
						f'<tr><td>Ezdxf</td><td>{ezdxf_version}</td></tr>'
						f'<tr><td>Lxml</td><td>{lxml_version}</td></tr>'
						f'<tr><td>Numpy</td><td>{numpy_version}</td></tr>'
						f'<tr><td>OpenGL</td><td>{OpenGLVersion}</td></tr>'
						f'<tr><td>Rasterio</td><td>{rasterio_version}</td></tr>'
						f'<tr><td>Rtree</td><td>{rtree_version}</td></tr>'
						f'<tr><td>Shapely</td><td>{shapely_version}</td></tr>'
						f'<tr><td>Simplejson</td><td>{simplejson_version}</td></tr>'
						f'<tr><td>Svglib</td><td>{svglib_version}</td></tr>'
						f'<tr><td>Vispy</td><td>{vispy_version}</td></tr>'
						'</table>'
					))

				self.components_tab = QtWidgets.QWidget()
				self.components_tab.setObjectName("components_about")
				self.components_tab_layout = QtWidgets.QVBoxLayout(self.components_tab)
				self.components_tab_layout.setContentsMargins(2, 2, 2, 2)
				add_components()
				self.components_tab_layout.addStretch()
				tab_widget.addTab(self.components_tab, _("Components"))
...
class MenuFileHandlers(QtCore.QObject):
	...
	def on_fileopengerber(self, signal, name=None):
		...
		if len(filenames) == 0:
			self.inform.emit(f'[WARNING_NOTCL] {_("Open Gerber")}: {_("Cancelled.")}')
		else:
	...
	def on_filerunscript(self, name=None, silent=False):
		...
		if name:
			filename = name
			if not self.app.cmd_line_headless:
				self.splash.showMessage('%s: %ssec\n%s' %
										(_("Canvas initialization started.\n"
