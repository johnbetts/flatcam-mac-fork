...
class FlatCAMObj(QtCore.QObject):
	...
	def add_properties_items(self, obj, treeWidget):
		...
		def job_thread(obj_prop):
			...
			# Only emit if still in normal mode (not in an editor).
			# The dims tree item may have been destroyed if the user entered
			# an editor while this worker was running.
	...
	def update_area_chull(self, area, length, width, chull_area, copper_area, location):
		...
		try:
			...
			log.debug("FlatCAMObj.update_area_chull() failed: %s" % str(e))
