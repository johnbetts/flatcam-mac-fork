...
class ObjectCollection(QtCore.QAbstractItemModel):
	...
	def data(self, index, role=None):
		...
		if role == Qt.ForegroundRole:
			...
			if obj:
				if obj.options["plot"]:
					return QtGui.QBrush(color) if(color := self.app.defaults['global_proj_item_color']) else QtWidgets.qApp.palette().windowText()
				return QtGui.QBrush(color) if(color := self.app.defaults['global_proj_item_dis_color']) else QtWidgets.qApp.palette().midlight()
			return index.internalPointer().data(index.column())
